...
export default function EmployeeAttendanceHistoryPage() {
  ...
  async function loadHistory(
    ...
    const selectedFrom = targetFrom || fromDate;
    const selectedTo = targetTo || toDate;
    ...
    try {
      ...
      const result = await apiRequest<HistoryResponse>(
        `/attendance/employee/${selectedEmployeeId}/history?from=${selectedFrom}&to=${selectedTo}`,
        {
          method: "GET",
          auth: true,
        },
      ...
      setMessage(error?.message || "Failed to load employee attendance history");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  async function initializePage() {
    ...
    try {
      ...
      if (firstEmployeeId) {
        await loadHistory(firstEmployeeId, fromDate, toDate);
      } else {
        setLoading(false);
      }
    } catch (error: any) {
  ...
  function handleRefresh() {
    loadHistory(employeeId, fromDate, toDate, true);
  }
  ...
  function getStatusStyle(status: AttendanceStatus): CSSProperties {
    ...
    return { ...styles.statusBadge, background: "#f3f4f6", color: "#374151" };
  }
    ...
    <div style={styles.page}>
      <aside style={styles.sidebar}>
        <div>
          ...
          <nav style={styles.nav}>
            <button type="button" style={styles.navItem} onClick={() => router.push("/dashboard")}>
              Dashboard
            </button>

            <button type="button" style={styles.navItem} onClick={() => router.push("/employees")}>
              Employees
            </button>
      ...
      <main style={styles.main}>
        <header style={styles.header}>
          <div>
            <h1 style={styles.pageTitle}>Employee Attendance History</h1>
            <p style={styles.pageSubtitle}>
              Review attendance records for a selected employee across a date range.
            </p>
          </div>
        ...
        <section style={styles.card}>
          <div style={styles.toolbar}>
            <div>
              ...
              <p style={styles.cardSubtitle}>
                Choose employee and date range to load attendance history.
              </p>
            </div>
          </div>
          ...
          <div style={styles.filterGrid}>
            ...
            <button type="button" onClick={handleApplyFilters} style={styles.primaryButton}>
              Load History
            </button>
          </div>
        </section>
        ...
        <section style={styles.statsGrid}>
          ...
          <div style={styles.statCard}>
            ...
            <div style={styles.statValue}>{stats.halfDay}</div>
          </div>
          ...
          <div style={styles.statCard}>
            ...
            <div style={styles.statValue}>{stats.missingPunch}</div>
          </div>

          <div style={styles.statCard}>
            <div style={styles.statLabel}>Worked</div>
            <div style={styles.statValueSmall}>{formatMinutes(stats.workedMinutes)}</div>
          </div>
        </section>
        ...
        <section style={styles.card}>
          <div style={styles.toolbar}>
            <div>
              <h2 style={styles.cardTitle}>History Records</h2>
              <p style={styles.cardSubtitle}>
                Search and filter returned attendance history records.
              </p>
            </div>
          </div>
          ...
          <div style={styles.recordFilterGrid}>
            ...
            <button type="button" onClick={handleResetFilters} style={styles.secondaryButton}>
              Reset Filters
            </button>
          </div>

          {loading ? (
            <div style={styles.emptyState}>Loading attendance history...</div>
          ) : filteredRecords.length === 0 ? (
            <div style={styles.emptyState}>
              No attendance history records found for the selected filters.
            </div>
          ) : (
